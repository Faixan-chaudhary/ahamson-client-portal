...
import { getClientLinkFromApi } from "@/lib/storage";
...
export function CreateLinkModal({ open, onClose, onCreated }: CreateLinkModalProps) {
  ...
  async function generate() {
    ...
    setError("");
    try {
      const res = await getClientLinkFromApi({
    ...
    <Modal open={open} onClose={handleClose}
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormField label="Client Company Name" required half>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2.5 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
              ...
              <div>
                ...
                <p className="text-emerald-600 text-xs flex items-center gap-1"><Timer className="w-3 h-3" />This link will expire in {formatLinkExpiry(linkExpireHours)}.</p>
              </div>
            </div>
            <div className="flex gap-2">
              <div className="flex-1 bg-[#F8F9FC] border border-[#0B1F3A]/12 rounded-xl px-3 py-2 text-[11px] text-[#0B1F3A] font-['JetBrains_Mono'] truncate">{generated}</div>
